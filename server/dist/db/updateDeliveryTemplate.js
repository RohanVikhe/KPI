...
async function main() {
    ...
    if (templates.length > 1) {
        console.error(`Multiple templates named "${TEMPLATE_NAME}" found. Resolve duplicates before running this script.`);
    ...
    const desiredMetricKeys = new Set(defaultTemplate.goals.flatMap((goal) => goal.metrics.map((metric) => metric.key)));
    ...
    console.log(`Updated template "${TEMPLATE_NAME}" (${template.id}). ` +
