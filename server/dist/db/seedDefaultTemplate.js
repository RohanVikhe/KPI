    ...
    goals: [
        {
            ...
            weight: 1,
            formula: "(on_time_delivery + scope_change_control + first_time_right + schedule_adherence + escalation_rate + post_delivery_defect_rate) / 6",
            metrics: [
                {
...
export async function seedDefaultTemplate(preferredCreatorId) {
    ...
    await prisma.$transaction(async (tx) => {
        ...
        const createdGoals = await Promise.all(defaultTemplate.goals.map((goal, index) => tx.kpiGoal.create({
            data: {
        ...
        const metricsData = createdGoals.flatMap((goal, goalIndex) => defaultTemplate.goals[goalIndex].metrics.map((metric, metricIndex) => ({
            templateId: template.id,
            goalId: goal.id,
            key: metric.key,
            label: metric.label,
