...
const aiClient = process.env.GEMINI_API_KEY
    ...
    : null;
export async function generateInsights(metrics, rawDeliveryData, subjectName = "the employee", context = {}) {
    if (!aiClient) {
        throw new AppError("AI Service is not configured. Please set GEMINI_API_KEY in your environment.", 503, "AI_NOT_CONFIGURED");
    }
...
`;
    try {
        ...
        if (error instanceof AppError)
            throw error;
        throw new AppError(`Failed to generate AI insights: ${error.message}`, 500, "AI_GENERATION_FAILED");
    }
}
