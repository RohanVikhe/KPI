...
function stripRawDeliveryMetadata(note) {
    if (!note)
        return note ?? null;
...
export async function generateSubmissionReport(submissionId, requesterId, requesterRole) {
    ...
    const normalizedUserName = submission.user.name
        ?.replace(/[<>:"/\\|?*\u0000-\u001F]/g, "")
        .replace(/\s+/g, " ")
        .trim() || "employee";
    ...
    const buffer = await new Promise((resolve, reject) => {
        ...
        const ensureSpace = (requiredHeight, continuationLabel) => {
        ...
        const drawSummaryField = (x, y, label, value, width) => {
        ...
        drawSummaryField(leftX, fieldY + fieldRowGap * 2, "Period", `${formatDate(submission.periodStart)} - ${formatDate(submission.periodEnd)}`, colWidth);
        ...
        submission.template.goals.forEach((goal, index) => {
            ...
            const rowHeight = Math.max(24, doc.heightOfString(goal.name, { width: summaryColumns.goal - 12 }), doc.heightOfString(notePreview, { width: summaryColumns.note - 12 })) + 10;
        ...
        submission.template.goals.forEach((goal, goalIndex) => {
            doc.addPage();
            doc.y = margin;
            drawSectionHeading("Detailed Goal Metrics", `Goal ${goalIndex + 1} of ${totalGoals}`);
            const goalScore = goalScoreMap.get(goal.id);
            ...
            goal.metrics.forEach((metric, metricIndex) => {
                ...
                const rowHeight = Math.max(doc.heightOfString(metric.label, { width: metricColumns.label - 12 }), doc.heightOfString(category, { width: metricColumns.category - 12 }), doc.heightOfString(valueLabel, { width: metricColumns.value - 12 })) + 12;
        ...
        doc.moveDown(0.65);
        doc.fillColor(palette.muted).font("Helvetica").fontSize(8).text("This report is system-generated from submitted KPI values and template formulas. For internal performance review use.", margin, doc.y, { width: contentWidth, align: "left" });
        doc.end();
    });
    return {
        fileName,
