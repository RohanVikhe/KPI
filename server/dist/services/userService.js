...
function shapeUserProjectsWithCounts(projects) {
    return projects.map((project) => {
        const { totalTickets, doneTickets, overdueTickets, changeRequestTickets, ...baseProject } = project;
        return {
            ...baseProject,
            ticketCounts: {
                total: totalTickets,
...
export async function listTeamUsers(managerId, options) {
...
export async function updateUser(id, input) {
    ...
    const nextManagerId = nextRole === Role.ADMIN ? null : managerProvided ? input.managerId : undefined;
...
async function assertKpiManagementAccess(requester, targetId) {
    const target = await prisma.user.findUnique({
        where: { id: targetId },
...
export async function deactivateUser(requester, targetId) {
    const target = await prisma.user.findUnique({
        where: { id: targetId },
...
export async function listUserKpiEntries(requester, targetId) {
...
export async function deleteUserKpiEntry(requester, targetId, submissionId) {
    await assertKpiManagementAccess(requester, targetId);
