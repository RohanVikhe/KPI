...
import { generateInsights, type MetricData, type RawDeliveryData } from "./aiService.js";

const RAW_DELIVERY_DATA_START = "[[RAW_DELIVERY_DATA_START]]";
const RAW_DELIVERY_DATA_END = "[[RAW_DELIVERY_DATA_END]]";

function extractRawDeliveryData(goalNotes: Array<{ note?: string | null }>): RawDeliveryData | null {
  for (const goalNote of goalNotes) {
    ...
    try {
      ...
      if (Array.isArray(parsed.headers) && Array.isArray(parsed.rows)) {
        return { headers: parsed.headers as string[], rows: parsed.rows as string[][] };
      }
    } catch {
      return null;
    }
...
export async function getInsights(req: Request, res: Response) {
  ...
  if (
    requestingUser.id !== submission.userId &&
  ...
  const metrics: MetricData[] = submission.values
...
export async function getSnapshotInsights(req: Request, res: Response) {
  const { employeeName, metrics, dateRange, isTeamView } = req.body as {
    employeeName?: string;
