...
export async function listTeamSubmissionsHandler(req: Request, res: Response) {
  if (!req.user) {
    throw new AppError("Authorization required", 401, "AUTH_REQUIRED");
  }
...
export async function listAllSubmissionsHandler(_req: Request, res: Response) {
...
export async function generateReportHandler(req: Request, res: Response) {
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="${report.fileName}"; filename*=UTF-8''${encodedFileName}`
  );
