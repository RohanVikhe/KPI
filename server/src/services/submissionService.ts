...
type SubmissionInput = {
  ...
  goalNotes?: GoalNoteInput[];
  rawDeliveryData?: RawDeliveryDataInput;
};

type SubmissionWithGoalScores = {
...
function buildRawDeliveryMetadata(rawDeliveryData: RawDeliveryDataInput): string {
  return `${RAW_DELIVERY_DATA_START}\n${JSON.stringify(rawDeliveryData)}\n${RAW_DELIVERY_DATA_END}`;
}

function splitRawDeliveryMetadata(note?: string | null) {
  if (!note) {
    return { cleanedNote: note ?? null, rawDeliveryData: null as RawDeliveryDataInput | null };
  }

  const startIndex = note.indexOf(RAW_DELIVERY_DATA_START);
  const endIndex = note.indexOf(RAW_DELIVERY_DATA_END);
  if (startIndex === -1 || endIndex === -1 || endIndex < startIndex) {
    return { cleanedNote: note, rawDeliveryData: null as RawDeliveryDataInput | null };
  }

  const jsonStart = startIndex + RAW_DELIVERY_DATA_START.length;
  const rawJson = note.slice(jsonStart, endIndex).trim();
  let rawDeliveryData: RawDeliveryDataInput | null = null;
...
function extractRawDeliveryDataFromGoalNotes<T extends { note?: string | null }>(goalNotes?: T[]) {
  if (!goalNotes || goalNotes.length === 0) {
    return { goalNotes, rawDeliveryData: null as RawDeliveryDataInput | null };
  }
...
function mergeRawDeliveryDataIntoGoalNotes(
  goalNotes: GoalNoteInput[],
  goals: Array<{ id: string; key: string }>,
  rawDeliveryData: RawDeliveryDataInput | null
) {
  if (!rawDeliveryData) {
    return goalNotes;
...
function attachGoalScores<
  T extends {
    template: any;
    values: SubmissionValueInput[];
    score?: number | null;
    goalNotes?: Array<{ note?: string | null }>;
  },
>(
  submission: T
): T & SubmissionWithGoalScores {
...
export async function createSubmission(userId: string, input: SubmissionInput) {
  ...
  const goalNoteInputs = mergeRawDeliveryDataIntoGoalNotes(
    input.goalNotes ?? [],
    template.goals.map((goal) => ({ id: goal.id, key: goal.key })),
    normalizedRawDeliveryData
  );
  if (goalNoteInputs.length > 0) {
    const goalIds = new Set(template.goals.map((goal) => goal.id));
    const providedGoalIds = new Set<string>();
