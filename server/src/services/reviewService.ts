import { ReviewStatus, Role, SubmissionStatus } from "@prisma/client";
...
function mapSubmissionStatus(status: ReviewStatus): SubmissionStatus {
  if (status === ReviewStatus.APPROVED) return SubmissionStatus.APPROVED;
  if (status === ReviewStatus.REJECTED) return SubmissionStatus.REJECTED;
