export type FlexibleDateParseResult =
  ...
  | { kind: "invalid"; code: "ambiguous" | "invalid"; message: string };

const DATE_PREFIX_PATTERN = /^(\d{1,4})[\/.-](\d{1,2})[\/.-](\d{1,4})(?:[T\s].*)?$/;

const toUtcDateOnly = (year: number, month: number, day: number) => new Date(Date.UTC(year, month - 1, day));
...
const expandYear = (year: number) => (year >= 70 ? 1900 + year : 2000 + year);

export const parseFlexibleDateString = (value: string | null | undefined): FlexibleDateParseResult => {
