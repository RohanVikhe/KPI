...
export function computeScore(input: ScoreInput): number | null {
  ...
  for (const metric of metrics) {
    ...
    const allowZeroForPositiveMinimum =
      rawValue === 0 && metric.min !== null && metric.min !== undefined && metric.min > 0;
    if (
      !allowZeroForPositiveMinimum &&
      metric.min !== null &&
  ...
  for (const metric of computedMetrics) {
    if (!metric.calcFormula || !metric.calcFormula.trim()) continue;
  ...
  if (hasFormula) {
    return evaluateFormula(formula as string, variables);
  }
...
export function computeMetricValues(metrics: KpiMetric[], values: MetricValueInput[]) {
  const valueMap = new Map(values.map((value) => [value.metricId, value.valueNumber]));
  const variables: Record<string, number> = {};
  ...
  for (const metric of metrics) {
    if (!metric.isComputed || !metric.calcFormula || !metric.calcFormula.trim()) continue;
    try {
      const computedValue = evaluateFormula(metric.calcFormula, variables);
      let finalValue = computedValue;
      if (metric.min !== null && metric.min !== undefined) {
